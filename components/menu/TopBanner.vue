<template>
  <header class="bar sticky top-0 z-20">
      ...
      <PopoverGroup class="hidden lg:flex lg:gap-x-12 textWhite lg:absolute lg:bottom-0 lg:right-10">
        ...
        <Popover class="relative">
          ...
          <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
            <PopoverPanel class="absolute -left-8 top-full z-10 mt-3 w-screen max-w-md overflow-hidden rounded-3xl bar shadow-lg ring-1 ring-gray-900/5">
              <div class="p-4">
                <div v-for="item in products" :key="item.name" class="group relative flex items-center gap-x-6 rounded-lg p-4 text-sm leading-6 hover:bg-gray-50">
                  ...
                  <div class="flex-auto">
                    <a :href="item.href" class="block font-semibold text-amber-50 hover:text-orange-400">
                      {{ item.name }}
    ...
    <Dialog as="div" class="lg:hidden " @close="mobileMenuOpen = false" :open="mobileMenuOpen">
      ...
      <DialogPanel class="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bar px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10  textWhite">
        ...
        <div class="mt-6 flow-root">
          <div class="-my-6 divide-y divide-gray-500/10">
            <div class="space-y-2 py-6">
              <a href="/" class="-mx-3 block rounded-lg py-2 px-3 text-base font-semibold leading-7 hover:text-orange-400 textWhite">Accueil</a>
              <Disclosure as="div" class="-mx-3" v-slot="{ open }">
                <DisclosureButton class="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base font-semibold leading-7 hover:text-orange-400">
                  Prestations
              ...
              <a href="/Actualites" class="-mx-3 block rounded-lg py-2 px-3 text-base font-semibold leading-7  hover:text-orange-400 textWhite">Actualités</a>
              <a href="/Contact" class="-mx-3 block rounded-lg py-2 px-3 text-base font-semibold leading-7  hover:text-orange-400 textWhite">Contact</a>
              <a href="#" class="-mx-3 block rounded-lg py-2.5 px-3 text-base font-semibold leading-7  hover:text-orange-400 textWhite">Espace membres</a>
            </div>
          </div>
        </div>
      </DialogPanel>
...
const products = [
  { name: 'Yogathérapie', description: 'Get a better understanding of your traffic', href: '/Yogatherapie', icon: ChartPieIcon },
  { name: 'Massages', description: 'Speak directly to your customers', href: '/massages', icon: CursorArrowRaysIcon },
  { name: 'Boite A Outils', description: 'Your customers’ data will be safe and secure', href: '/BoiteAOutils', icon: FingerPrintIcon },
]
const callsToAction = [
